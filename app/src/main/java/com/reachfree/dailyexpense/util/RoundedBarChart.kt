...
class RoundedBarChart : BarChart {
    ...
    private fun readRadiusAttr(context: Context, attrs: AttributeSet) {
        val a: TypedArray =
            context.theme.obtainStyledAttributes(attrs, com.reachfree.dailyexpense.R.styleable.RoundedBarChart, 0, 0)
        try {
            setRadius(a.getDimensionPixelSize(com.reachfree.dailyexpense.R.styleable.RoundedBarChart_radius, 0))
        } finally {
            a.recycle()
        }
    }
        ...
        override fun drawDataSet(c: Canvas, dataSet: IBarDataSet, index: Int) {
            ...
            if (mChart.isDrawBarShadowEnabled) {
                ...
                while (i < count) {
                    ...
                    c.drawRoundRect(mBarShadowRectBuffer, mRadius.toFloat(),
                        mRadius.toFloat(), mShadowPaint)
                    i++
                }
            }
